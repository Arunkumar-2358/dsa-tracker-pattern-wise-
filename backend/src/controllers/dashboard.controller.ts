...
export const getDashboard = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const solved = userProblems.filter((up) =>
      ['SOLVED', 'REVISED', 'MASTERED'].includes(up.status)
    ).length;
    ...
    const revisionDue = await prisma.userProblem.count({
      where: {
        userId,
        status: 'REVISED',
